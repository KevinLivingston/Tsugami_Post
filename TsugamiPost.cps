...
function onOpen() {
  ...
  if (getProperty("machineType") == "PUMA_MX") {
    writeBlock(
      gFormat.format(0),
      gFormat.format(18),
...
function formatFeedMode(mode) {
  var fMode = (mode == FEED_PER_REVOLUTION) ? getCode("FEED_MODE_MM_REV") : getCode("FEED_MODE_MM_MIN");
  if (fMode) {
...
function invertAxes(activate, polarMode) {
  if (activate) {
    var scaleValue = getProperty("reverseAxes") ? (polarMode ? 1 : -1) : 1;
...
function onSection() {
  ...
  if (tempSpindle == SPINDLE_LIVE) {
    if (insertToolCall || wcsOut || feedMode) {
      ...
      if ((gotBAxis && (activeTurret != 2)) && !bAxisIsManual) {
        if (useG368(abc) && !machineState.usePolarMode && !machineState.useXZCMode) {
  ...
  if (insertToolCall && (!machineState.stockTransferIsActive || (lastSpindleMode != SPINDLE_CONSTANT_SURFACE_SPEED))) {
...
function getHighfeedrate(radius) {
  if (gFeedModeModal.getCurrent() == getCode("FEED_MODE_MM_REV", getSpindle(TOOL))) {
    if (toDeg(radius) <= 0) {
...
function onRapid(_x, _y, _z) {
  ...
  var z = zOutput.format(_z);
  if (x || y || z) {
    var useG1 = (((!getProperty("useG0") && ((((x ? 1 : 0) + (y ? 1 : 0) + (z ? 1 : 0)) > 1))) || machineState.usePolarMode) && !isCannedCycle);
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (!machineState.stockTransferIsActive) {
      ...
      onCommand(COMMAND_UNLOCK_MULTI_AXIS);
      gFeedModeModal.reset();
      var feedMode = gFeedModeModal.format(getCode("FEED_MODE_MM_MIN", getSpindle(TOOL)));
      gPlaneModal.reset();
      if (!getProperty("optimizeCaxisSelect")) {
        cAxisEngageModal.reset();
      }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) { // first cycle point
    ...
    var F = (gFeedModeModal.getCurrent() == getCode("FEED_MODE_MM_REV", getSpindle(TOOL)) || tapping) ? cycle.feedrate / spindleSpeed : cycle.feedrate;
    ...
    case "left-tapping":
      ...
      writeBlock(gFeedModeModal.format(getCode("FEED_MODE_MM_REV", getSpindle(TOOL))));
...
function onCycleEnd() {
  if (!cycleExpanded && !machineState.stockTransferIsActive) {
    ...
    var fMode = tapping ? gFeedModeModal.format(getCode("FEED_MODE_MM_MIN", getSpindle(TOOL))) : "";
...
function moveSubSpindle(_method, _position, _feed, _useMachineFrame, _comment, _error) {
  ...
  case TORQUE:
    ...
    gMotionModal.reset();
    writeBlock(
      gFeedModeModal.format(getCode("FEED_MODE_MM_MIN", getSpindle(TOOL))),
      gMotionModal.format(1),
      gFormat.format(31),
      pOutput.format(99),
